...
class ConfigManager:
    ...
    def _load_config_file(self) -> configparser.ConfigParser:
        ...
        if not files_read:
            raise ConfigurationError(f"Could not read config file at: {self.config_path}")
    ...
    def _create_paths(self) -> Paths:
        ...
        """
        paths = Paths(
            template_path=os.path.join(self.script_dir, self.config['Paths']['template_path']),
            input_dir=os.path.join(self.script_dir, self.config['Paths']['input_dir']),
            output_dir=os.path.join(self.script_dir, self.config['Paths']['output_dir']),
