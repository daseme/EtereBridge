...
class FileProcessor:
    ...
    def apply_transformations(
        ...
        try:
            # Set billcode
            billcode = (
                f"{text_box_180}:{text_box_171}"
                if text_box_180 and text_box_171
                else text_box_171 or text_box_180 or ""
            )
            ...
            logging.info(
                f"Applying market replacements: {self.config.market_replacements}"
            )
            ...
            if "Gross Rate" in df.columns:
                ...
                df["Gross Rate"] = (
                    df["Gross Rate"].apply(self.safe_to_numeric).fillna(0)
                )
            ...
            if "Line" in df.columns:
                df["Line"] = (
                    pd.to_numeric(df["Line"], errors="coerce").fillna(0).astype(int)
                )
